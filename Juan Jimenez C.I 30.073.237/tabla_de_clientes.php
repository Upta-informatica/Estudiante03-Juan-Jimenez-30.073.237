...
<div class="container" id="caja">
	 <table class="table">
	 	<thead>
          ...
          <td><a href="Formulario_clientes.php"><button type="button" class="btn btn-primary">Crear Cliente</button></a></td>	
		  </tr>
		</thead>
		<tbody>
	<?php
  ...
  if($filas!=null){foreach ($filas as $fila){?>
     ...
     <td class="text-center"><?php echo date("d/m/Y",strtotime($fila['fecha_add']))?></td>
      <?php echo "
     <td> 
     <a href='eliminar_clientes.php?id=".$fila['id']."'><button type='button' class='btn btn-danger'>eliminar</button></a>
     <a href='modificar_cliente.php?id=".$fila['id']."'><button type='button' class='btn btn-success'>modificar</button></a>
     </td>"?>
		</tr>
<?php
  }
